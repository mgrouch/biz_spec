...
\usepackage{array}
\usepackage{tabularx}
% Optional: nicer wrapping in tabularx
\usepackage{ragged2e}
...
This paper presents a \emph{structural reconstruction} of CNS-style continuous net settlement: a reference specification that makes the core mechanics explicit and amenable to state--transition analysis. The model is informed by publicly described clearing workflows (e.g., NSCC/DTCC) but is not a line-by-line reproduction of any proprietary system or rulebook, and deliberately omits institution-specific exceptions and operational details. Instead, it isolates the key invariants, admissibility interfaces, and allocation degrees of freedom needed for simulation, comparison, and formal reasoning.
...
The CNS dynamics defined above satisfy a Markov property with respect to the state $\sigma_t$: conditioned on the current state and the exogenous inputs $(E_t, D_t, K_t, \Pi_t)$, the distribution of the next state $\sigma_{t+1}$ is independent of the prior history. All history-dependent features (e.g., rolling fails, aging, margin exhaustion, and bounded lookback statistics) are explicitly encoded in the state variables, allowing the system to be analyzed as a controlled discrete-time Markov process.
\end{remark}
...
\begin{proof}
By induction: base case holds by assumption. 

If $\sum_p X_t(p,s)=0$ and $\sum_p \Delta X^{tr}_t(p,s)=0$, then $\sum_p X^{(1)}_t(p,s)=0$.
...
B_{t+1}(p,s)=
\begin{cases}
1, & \mathrm{ReqMargin}_t(p) > M_t(p),\\
B_t(p,s), & \text{otherwise}.
\end{cases}
\]

\[
M_{t+1}(p)=\max\!\bigl\{0,\; M_t(p)-\mathrm{ReqMargin}_t(p)\bigr\}.
\]
...
\begin{alignedat}{2}
\text{Key}_t(p,s) = \bigl(
  -&\text{Age}_t(p,s), \quad &&\text{Class}(p),\\
  -&\text{RoundNeed}_t(p,s), \quad &&\text{FailScore}_t(p),\\
  &\text{TieID}(p)
\bigr)
\end{alignedat}
\]
...
\[
\begin{aligned}
\text{Del}_0(B,X)&=150,\quad \text{Del}_0(C,X)=100,\quad \\
\text{TotDel}_0(X)&=250.
\end{aligned}
\]
After deliveries:
...
\[
\begin{aligned}
\text{NeedDel}_1(B,X)&=50,\quad \text{Del}_1(B,X)=\min(30,50)=30,\quad \\
\text{TotDel}_1(X)&=30.
\end{aligned}
\]
After deliveries:
...
\subsubsection{Policy (c): Deterministic priority allocation}\par
...
\subsubsection{Policy (b): Pure randomized allocation (example draw)}\par
...
\subsubsection{Policy (hybrid): Probabilistic priority (softmax-biased)}\par

Use score components:
\[
\begin{aligned}
k_1(p,s) &= -\mathrm{Age}_t(p,s), \quad
k_2(p,s) = \mathrm{Class}(p), \\
k_3(p,s) &= -\mathrm{RoundNeed}_t(p,s)\\
(\text{and if }\mathrm{LotSize}(s)&=1,\ \mathrm{RoundNeed}_t=\mathrm{NeedRec}_t),\\
k_4(p,s) &= \mathrm{FailScore}_t(p).
\end{aligned}
\]
...
\[
\begin{aligned}
\text{Score}_1(A,X)&=1\cdot(-1)+0.2\cdot 1=-0.8,\\
...
% ===================== COMPARISON =====================
\section{Comparative Reference Specification Perspective}
Table~\ref{tab:cns-rtgs-dlt} situates the CNS reference specification developed in this paper alongside canonical RTGS (Real-Time Gross Settlement) and ledger-based (DLT - Distributed Ledger Technology) reference models, emphasizing differences in state structure, transition semantics, and policy degrees of freedom rather than implementation detail.

\begin{table*}[t]
\centering
\caption{Structural comparison of settlement architectures as reference state--transition systems}
\label{tab:cns-rtgs-dlt}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.1}
\begin{tabularx}{\textwidth}{lXXX}
\toprule
\textbf{Dimension} & \textbf{CNS} & \textbf{RTGS} & \textbf{DLT / ledger-based} \\
\midrule
Settlement granularity & Multilateral netting; rolling obligations & Gross; immediate or queued & Transaction- or block-level ordering \\
State representation & Obligation vectors $(X_t, C_t)$ + controls $(M_t,B_t)$ & Account balances + payment queues & Replicated ledger state + protocol metadata \\
Time model & Discrete cycles (e.g., business days) & Event-driven / continuous processing & Discrete rounds / blocks; finality notion \\
Liquidity usage & Reduced via netting (depends on fails/caps) & Higher; prefunding or intraday liquidity & Design-dependent (tokens, liquidity bridges) \\
Failure handling & Fails persist and roll forward & Rejection or queueing; partial processing & Forks/reorgs or finality delays (protocol-dependent) \\
Allocation freedom & Policy-defined allocation among competing claims & None (execution order rules) & Protocol-defined ordering and inclusion rules \\
Risk controls locus & Explicit operator $g$ (margin, blocks, throttles) & Credit limits, prefunding, gatekeeping rules & Protocol rules, collateral/slashing (if applicable) \\
Cash treatment & Capacity-based model; operational asymmetries via $K_t$ and $g$ & Central-bank money / settlement account model & Native tokens or bridged cash rails \\
Determinism vs.\ randomness & Deterministic or stochastic policy modules & Deterministic processing logic & Often probabilistic leader/committee selection \\
Markov view & Controlled Markov process (state-augmented) & Markov w.r.t.\ balances/queues & Markov w.r.t.\ ledger/protocol state \\
\bottomrule
\end{tabularx}
\end{table*}
...
This work was prepared with the supervised assistance of artificial intelligence tools, including ChatGPT and DeepSeek, which were employed to aid in mathematical drafting, and editorial preparation. All conceptual development, and final revisions were conducted independently by the author.
