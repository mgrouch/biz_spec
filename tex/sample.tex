...
\begin{enumerate}[leftmargin=*]
  ...
  \item \khead{Crash Recovery}: The system must recover deterministically from failures without losing allocated state using \kfeat{write-ahead logging (WAL)}.
...
For any transition that mutates persistent state, the system must append a corresponding record to $L_s$ \emph{before} the mutation becomes visible. This discipline ensures crash recovery: after a crash and restart, the system can reconstruct the exact state by replaying all log records in order, starting from the initial empty state.
...
Each log record captures sufficient information to reconstruct the state change during recovery. For example, $\mathsf{CarryInc}(s,\delta)$ records both the security identifier and the quantity of settlement supply that became available.
...
The following transitions define the core operational behavior of each security shard. Each transition follows the write-ahead logging discipline: the corresponding log record is appended to $L_s$ \emph{before} the state mutation takes effect.
...
\textbf{Processing:} The transition records the new CNS obligation $x$ for security $s$ by appending $\mathsf{CNSSet}(s,x)$ to the log, then updates $O_s$:
\[
...
\]
The $\delta$ represents securities that have become available for allocation. If $O_s$ increases (or becomes more negative), $\delta=0$, meaning no new supply is generated.

Then append two log records: $\mathsf{CarryInc}(s,\delta)$ to record the new supply, and $\mathsf{CNSSet}(s,x)$ to record the new obligation:
\[
...
\textbf{Processing:} Append $\mathsf{TradeAdded}(t)$ to the log, then add the trade to the trade set:
\[
...
\begin{enumerate}[leftmargin=*,topsep=0.25em,itemsep=0.15em]
  ...
  \item Appends $\mathsf{AllocationCreated}(id_t,t,s,X_t)$ to the log
...
\textbf{Processing:} Append $\mathsf{Published}(id)$ to the log, then update publication status:
\[
...
\textbf{Publication Interface Assumption:} We assume the lending system provides idempotent processing: repeated publication attempts with the same $id$ have the same effect as a single publication. This is typically implemented by the lending system maintaining a record of processed allocation identifiers.

\textbf{Retry Semantics:} In practice, publication may fail due to network issues. The system can retry pending publications until successful. The idempotence guarantee ensures retries cause no harm.
...
\]
The $\bot_s$ state represents a crashed shard where only persistent storage (log and possibly checkpoints) survives. Volatile runtime state is lost.
...
\textbf{Recovery via Replay:} On restart, the system reconstructs state by deterministic replay of $L_s$ starting from initial state $\sigma_{0,s}$:
\begin{itemize}[leftmargin=*,topsep=0.25em,itemsep=0.15em]
  \item $\mathsf{TradeAdded}(t)$: Add trade $t$ to $Tr_s$
  \item $\mathsf{CNSSet}(s,x)$: Set $O_s := x$
  \item $\mathsf{CarryInc}(s,\delta)$: Increase $U_s$ by $\delta$
  \item $\mathsf{AllocationCreated}(id,t,s,q)$: Add allocation $(id,t,q)$ to $A_s$; update $qty^{alloc}(t)$; decrease $U_s$ by $q$; set $P_s(id):=\mathsf{pending}$ if undefined
  \item $\mathsf{Published}(id)$: Set $P_s(id):=\mathsf{published}$
\end{itemize}

\textbf{Replay Well-Formedness:} The log is assumed to be well-formed: allocations never cause $U_s$ to become negative, and each allocation references a trade that exists (was previously added via $\mathsf{TradeAdded}$). This is guaranteed by the write-ahead discipline at runtime.

\textbf{Checkpointing Optimization:} In practice, periodic checkpoints (snapshots of full state) can accelerate recovery by allowing replay to start from a recent checkpoint rather than from initial state.
