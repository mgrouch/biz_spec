...
\begin{enumerate}[leftmargin=*,topsep=0.25em,itemsep=0.15em]
  \item persists the resulting shard state updates 
\hfill\break(e.g., $A_s,U_s,P_s,O_s,Tr_s$),
...
\qquad
X \in \mathbb{Z}_{\ge 0}^{Tr_s},
\]
where each component $X_t$ is the quantity allocated to trade $t$.

\textbf{Feasibility Constraints:} The strategy must respect:
\[
\forall t\in Tr_s:\ 0 \le X_t \le open_s(t),
\qquad
...
For any output plan
\hfill\break
 $X=\mathcal{F}_{s,\Theta}(Tr_s,Open_s,U_s)$:
\begin{enumerate}[leftmargin=*]
