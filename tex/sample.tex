...
qty^{tot}(t) &\in \mathbb{Z}, &&\text{total quantity to be settled (signed: $>0$ receive, $<0$ deliver)}
\end{aligned}
\]
...
\]
This $Need_s$ represents the \emph{net} open settlement demand from all client trades for security $s$ (signed: receive positive, deliver negative). It guides allocation decisions and appears in the end-of-day reconciliation equation.
...
For any output plan $X=\mathcal{F}_{s,\Theta}(Tr_s,Open_s,U_s)$, let $d\triangleq \sgn(U_s)$:
\begin{enumerate}[leftmargin=*]
