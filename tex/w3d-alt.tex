...
Full Math, Discretization, and Implementation-Traceable Algorithms (No Cross-Refs)
}
...
It includes continuous-time models, discrete-time propagation with closed-form oscillator transitions (under/critical/over-damped)
and Simpson-rule noise integration, measurement Jacobians, NIS gating, a Joseph-form covariance update, warmup/bias gating policies,
and an optional stabilization approximation that enforces axis-independent covariance by removing cross-axis blocks.
\end{abstract}

% ==============================================================================
\section{Frames and Conventions}
\paragraph{Frames.}
WORLD is typically NED: $+Z$ is down. BODY' is an un-heeled virtual body frame (heel can be applied externally; the implementation can also de-heel IMU vectors internally).
...
\dth \leftarrow \0.
\]
All Jacobians below assume this same convention.
...
Let $K$ denote the number of oscillator modes (\texttt{KMODES}). For each mode $k\in\{1,\dots,K\}$, the wave block stores $\pvec_k,\vvec_k\in\R^3$ in WORLD.
...
\[
\dot{\dth} = -\skew{\w}\,\dth - \delta\bg - \mathbf{n}_g,\qquad
\dot{\delta\bg} = \mathbf{n}_{bg},
\]
where $\mathbf{n}_g$ is gyro noise and $\mathbf{n}_{bg}$ is gyro-bias random walk. If gyro bias is disabled, drop the bias terms.
...
\]
where $u_0=\mathbf{u}(0)$, $u_m=\mathbf{u}(\Ts/2)$, and $u_1=\mathbf{u}(\Ts)$.

\subsection{$6\times6$ assembly for mode $k$ (matches p-then-v ordering)}
For each mode $k$, assemble the $6\times6$ transition from per-axis $2\times2$ entries:
\[
\Phi_{6,k}=\begin{bmatrix}A & B\\ C & D\end{bmatrix},
\]
with diagonal $3\times3$ blocks
\[
A=\diag(\Phi_{00}^x,\Phi_{00}^y,\Phi_{00}^z),\quad
B=\diag(\Phi_{01}^x,\Phi_{01}^y,\Phi_{01}^z),\quad
C=\diag(\Phi_{10}^x,\Phi_{10}^y,\Phi_{10}^z),\quad
D=\diag(\Phi_{11}^x,\Phi_{11}^y,\Phi_{11}^z),
\]
and similarly assemble $Q_{d,6,k}$ from the per-axis $Q_d$ elements using the same block layout.
...
\[
\qref \leftarrow \qref \otimes \exp\!\left(\frac{1}{2}\dth\right),\qquad
\dth\leftarrow \0.
\]
...
\State Compute $\Phi(\Ts)$ (under/critical/over-damped closed form)
...
\begin{itemize}
\item Quaternion convention: $\qref$ is WORLD$\rightarrow$BODY' and correction is right-multiply.
...
\item Wave acceleration is computed in WORLD from oscillator states and rotated into BODY' only inside measurement prediction.
\item Accelerometer attitude Jacobian uses $\delta(\RwB\uvec)=-\skew{\RwB\uvec}\dth$.
\item Covariance update uses Joseph-equivalent form plus symmetrize/clamp/PSD for numerical stability.
\end{itemize}
